...
/** `DELETE /api/taxonomy/education-levels/:id` — requires `taxonomy.delete`. Success is `204` (no
 * body), matching legacy's `ParseIntPipe`'s `id`-must-be-an-integer behavior via `requireIntFromQuery`
 * (the path segment arrives as a string regardless of route shape). */
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  return withTenantContext(request, async () => {
    ...
    await getTaxonomyService().deleteEducationLevel(requireIntFromQuery(id, 'id'));
