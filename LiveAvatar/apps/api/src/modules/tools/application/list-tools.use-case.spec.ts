...
describe('ListToolsUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new ListToolsUseCase(tenants, tools);
  });
  ...
  it('returns every tool for the tenant, enabled or not', async () => {
    tenants.findById.mockResolvedValue(makeTenant());
    tools.listByTenant.mockResolvedValue([makeToolRecord(), makeToolRecord({ id: 'tool-2', enabled: false })]);
