...
/** True if `error` is a MySQL duplicate-key violation — `FinalizeExamService.finalize` translates this
 * into `ExamTypeNameExistsError` (FR-AUTH-1: "duplicate exam type names within the tenant are
 * rejected"), the identical race `ExamAuthoringService.createFromZip` already guards against. */
