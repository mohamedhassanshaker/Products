...
describe('GetToolUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new GetToolUseCase(tenants, tools);
  });

  it('404s an unknown tenant', async () => {
    tenants.findById.mockResolvedValue(null);
    await expect(useCase.execute(actor, 'tenant-1', 'tool-1')).rejects.toMatchObject({ code: 'TENANT_NOT_FOUND' });
  });
  ...
  it('404s an unknown tool id', async () => {
    ...
    await expect(useCase.execute(actor, 'tenant-1', 'nope')).rejects.toMatchObject({ code: 'TOOL_NOT_FOUND' });
  });

  it('returns the tool DTO', async () => {
    tenants.findById.mockResolvedValue(makeTenant());
