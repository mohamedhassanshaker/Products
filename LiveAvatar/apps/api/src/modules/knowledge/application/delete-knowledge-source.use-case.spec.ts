...
import { DeleteKnowledgeSourceUseCase } from './delete-knowledge-source.use-case';
...
describe('DeleteKnowledgeSourceUseCase', () => {
  ...
  let useCase: DeleteKnowledgeSourceUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new DeleteKnowledgeSourceUseCase(tenants, sources);
  });
  ...
  it('deletes a known source', async () => {
    tenants.findById.mockResolvedValue(makeTenant());
    sources.findById.mockResolvedValue(makeSourceRecord());
    sources.delete.mockResolvedValue(true);
    await useCase.execute(actor, 'tenant-1', 'source-1');
    expect(sources.delete).toHaveBeenCalledWith('tenant-1', 'source-1');
  });

  it('404s if the row disappears between the existence check and the delete', async () => {
    tenants.findById.mockResolvedValue(makeTenant());
    sources.findById.mockResolvedValue(makeSourceRecord());
    sources.delete.mockResolvedValue(false);
    await expect(useCase.execute(actor, 'tenant-1', 'source-1')).rejects.toMatchObject({
      code: 'KNOWLEDGE_SOURCE_NOT_FOUND',
    });
  });
});
