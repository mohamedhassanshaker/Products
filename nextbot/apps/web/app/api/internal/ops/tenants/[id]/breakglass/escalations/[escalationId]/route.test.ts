...
const getEscalationDetailMock = vi.fn();
vi.mock("@nextbot/escalations", () => ({
  getEscalationDetail: (...a: unknown[]) => getEscalationDetailMock(...a),
}));

function makeRequest() {
  return new NextRequest("http://localhost/api/internal/ops/tenants/t1/breakglass/escalations/e1");
}

describe("GET /api/internal/ops/tenants/[id]/breakglass/escalations/[escalationId] (Phase 20, FR-ADM-09)", () => {
  beforeEach(() => {
    requirePlatformApiMock.mockReset();
    requireActiveBreakglassTenantContextMock.mockReset();
    getEscalationDetailMock.mockReset();
  });
  ...
  it("returns the guard's Response verbatim when unauthorized", async () => {
    ...
    const res = await GET(makeRequest(), { params: Promise.resolve({ id: "t1", escalationId: "e1" }) });
    expect(res).toBe(deny);
  });

  it("returns 404 when the escalation doesn't resolve within this tenant", async () => {
    ...
    getEscalationDetailMock.mockResolvedValue(null);
    const { GET } = await import("./route.js");
    const res = await GET(makeRequest(), { params: Promise.resolve({ id: "t1", escalationId: "e1" }) });
    expect(res.status).toBe(404);
  });

  it("returns the escalation detail (with the internal `row` field stripped) once the grant is active", async () => {
    requirePlatformApiMock.mockResolvedValue({ authorized: true });
    const ctx = { tenantId: "t1" };
    requireActiveBreakglassTenantContextMock.mockResolvedValue({ ctx, grantId: "g1" });
    getEscalationDetailMock.mockResolvedValue({ id: "e1", row: { secretInternalField: true }, transcriptExcerpt: [] });
    const { GET } = await import("./route.js");
    const res = await GET(makeRequest(), { params: Promise.resolve({ id: "t1", escalationId: "e1" }) });
    ...
    expect(getEscalationDetailMock).toHaveBeenCalledWith(ctx, "e1");
  });
});

describe("unimplemented verbs on /api/internal/ops/tenants/[id]/breakglass/escalations/[escalationId]", () => {
